...
'MODULE FE (FMOD EASY)
'added variables
Public mp3channel As Long
Public mp3stream As Long
Public modmodule As Long
...
Public Const FSOUND_INIT_ACCURATEVULEVELS = &H8      'This latency adjusts FSOUND_GetCurrentLevels, but incurs a small cpu and memory hit
Public Const FSOUND_INIT_DISABLE_CORE0_REVERB = &H10 'PS2 only - Disable reverb on CORE 0 to regain SRAM
Public Const FSOUND_INIT_DISABLE_CORE1_REVERB = &H20 'PS2 only - Disable reverb on CORE 1 to regain SRAM
...
Public Declare Sub FSOUND_GetMemoryStats Lib "fmod.dll" Alias "_FSOUND_GetCPUUsage@0" (ByRef currentalloced As Long, ByRef maxalloced As Long)
...
If modmodule <> 0 Then
    FMUSIC_StopSong modmodule
    FMUSIC_FreeSong modmodule
    modmodule = 0
End If
...
Public Sub OPENfile(file As String, blooping As Boolean)
...
If mp3stream <> 0 Then
    ...
    mp3stream = 0
End If
'The Line That Opens The File
If blooping Then
    mp3stream = FSOUND_Stream_OpenFile(file, FSOUND_STEREO Or FSOUND_2D Or FSOUND_16BITS Or FSOUND_LOOP_NORMAL, 0)
    Else
    mp3stream = FSOUND_Stream_OpenFile(file, FSOUND_STEREO Or FSOUND_2D Or FSOUND_16BITS, 0)
End If
End Sub
...
mp3channel = FSOUND_Stream_Play(FSOUND_FREE, mp3stream)
'FMod.FSOUND_SetLoopMode sarray(2, channel), 23

End Sub

Public Sub STOPfile()
'stop stream
    FSOUND_Stream_Stop mp3stream ' spira pesenta
    FSOUND_StopSound mp3channel ' osvobojdava kanala
    mp3channel = 0 '- sled stop kanala se osvobojdava i za tova e redno promenlivata da e 0
End Sub

Public Sub PAUSEfile()
'no error if no stream loaded or playing !! !!!!
FSOUND_SetPaused mp3channel, Not CBool(FSOUND_GetPaused(mp3channel))
End Sub
...
Public Sub VOLUMEfile(volume As Byte)
FSOUND_SetVolume mp3channel, volume
End Sub
...
Public Function OPENmus(file As String) As Boolean
If modmodule <> 0 Then FMUSIC_StopSong modmodule
modmodule = 0
modmodule = FMUSIC_LoadSong(file)
If (modmodule = 0) Then OPENmus = False _
 Else OPENmus = True
End Function
...
If modmodule <> 0 And Not FMUSIC_IsPlaying(modmodule) Then
FMUSIC_SetLooping modmodule, bloop
FMUSIC_PlaySong (modmodule)
End If
End Sub

Public Sub PAUSEmus()
'it doesn't make an error if no music is loaded
FMUSIC_SetPaused modmodule, Not CBool(mFMod.FMUSIC_GetPaused(modmodule))
End Sub
...
Public Sub VOLUMEmus(volume As Long)
' 0-256
FMUSIC_SetMasterVolume modmodule, volume
'FMod.FSOUND_SetVolume modmodule, volume
End Sub
